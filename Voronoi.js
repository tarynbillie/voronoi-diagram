...
Voronoi.prototype.InsertParabola = function(p) {
	if(!this.root) {
		this.root = new Parabola(p); this.fp = p; return;
	}
	...
	if(this.root.isLeaf && this.root.site.y - p.y < 0.01) {
		...
		if(p.x > this.fp.x) this.root.edge = new Edge(s, this.fp, p);
		else this.root.edge = new Edge(s, p, this.fp);
...
Voronoi.prototype.RemoveParabola = function(e) {						
	...
	if(p0.cEvent) {
		this.queue.remove(p0.cEvent); p0.cEvent = null;
	}
	if(p2.cEvent) {
		this.queue.remove(p2.cEvent); p2.cEvent = null;
	}
